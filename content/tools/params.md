...
| builder (bool)               | *Apriltag location solver parameter*. Will localize origin tag from a networked solver. All other tags found will be updated or created in the ATLAS (e.g ```builder=true```)     |
...
| networkedTagSolver (bool)    | *Apriltag location solver parameter*. Either set ```networkedTagSolver``` or ```publishDetections```. When true, publishes tag detections (to ```realm/g/a/camera-name```) **and defers all tag solving of client camera to a solver sitting on pubsub**|
| publishDetections (bool)     | *Apriltag location solver parameter*. Either set ```networkedTagSolver``` or ```publishDetections```. When true, publishes tag detections (to ```realm/g/a/camera-name```); **still processes the tag and relocalizes accordingly**|
