...
| :--- | :--- | :--- | :--- | :--- |
| **acceleration** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | For sphere and disc distributions, only the x axis is used. | No |
...
| **accelerationSpread** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | Spread of the particle's acceleration. for sphere and disc distributions, only the x axis is used. | No |
...
| **angle** | number[] | `[0]` | 2D rotation of the particle over the particle's lifetime, max 4 elements. | No |
...
| **colorSpread** | [Vector3](attr-vector3)[] | `[]` | Spread to apply to colors, spread an array of vec3 (r g b) with 0 for no spread. note the spread will be re-applied through-out the lifetime of the particle. | No |
...
| **opacity** | number[] | `[1]` | Opacity over the particle's lifetime, max 4 elements. | No |
...
| **positionOffset** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | Fixed offset to the apply to the emitter relative to its parent entity. | No |
| **positionSpread** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | Particles are positioned within +- of these local bounds. for sphere and disc distributions only the x axis is used. | No |
...
| **radiusScale** | [Vector3](attr-vector3) | `{'x': 1, 'y': 1, 'z': 1}` | Scales the emitter for sphere and disc shapes to form oblongs and ellipses. | No |
...
| **rotationAxis** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | Local axis when using rotation. | No |
| **rotationAxisSpread** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | Variance in the axis of rotation. | No |
...
| **textureFrames** | [Vector2](attr-vector2) | `{'x': 1, 'y': 1}` | X and Y frames for a spritesheet. each particle will transition through every frame of the spritesheet over its lifetime (see textureFramesLoop). | No |
| **useTransparency** | boolean | `True` | Should the particles be rendered with transparency? | No |
| **velocity** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | For sphere and disc distributions, only the x axis is used. | No |
...
| **velocitySpread** | [Vector3](attr-vector3) | `{'x': 0, 'y': 0, 'z': 0}` | Variance for the velocity. | No |
