...
| **loop** | ['string', 'integer', 'boolean'] | `0` | How many times the animation should repeat. If the value is true, the animation will repeat infinitely. | No |
| **pauseEvents** | string[] | `[]` | Comma-separated list of events to listen to trigger pause. Can be resumed with resumeEvents. | No |
