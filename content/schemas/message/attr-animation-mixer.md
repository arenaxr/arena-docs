...
**Used by:** [GLTF Model](obj-gltf-model)
...
| **useRegExp** | boolean | `False` | If true, the clip property will be treated as a regular expression instead of a wildcard pattern. | No |
...
| **repetitions** | number |  | Number of times to play the clip, in addition to the first play (null = Infinity). Repetitions are ignored for loop: once. | No |
