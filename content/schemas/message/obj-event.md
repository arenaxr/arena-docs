---
title: event
layout: default
parent: Objects Schema
grand_parent: ARENA Objects
---

<!--CAUTION: This file is autogenerated from https://github.com/arenaxr/arena-schemas. Changes made here may be overwritten.-->

# `event`

Generate an event message for an object.

This is the schema for Event, the properties of wire object type `event`.


All wire objects have a set of basic [ARENA Message](arena-message) envelope attributes: `{object_id, action, type, persist, data}`.
- The `type` attribute must be set to `"mousedown"` for this wire object.
- The `data` attribute defines the `event` object-specific attributes listed below.


### Event Properties

| Attribute | Type | Default | Description | Required |
| :--- | :--- | :--- | :--- | :--- |
| **target** | string |  | The `object_id` of event destination. | Yes |
| **targetPosition** | [Vector3](attr-vector3) |  | The event destination position in 3D. | Yes |
| **originPosition** | [Vector3](attr-vector3) | `{'x': 0, 'y': 1.6, 'z': 0}` | The event origination position in 3D. | No |
| **source** | ~~string~~ | ~~~~ | ~~DEPRECATED: data.source is deprecated for clientEvent, use data.target instead.~~ | ~~No~~ |
| **position** | ~~object~~ | ~~~~ | ~~DEPRECATED: data.position is deprecated for clientEvent, use data.targetPosition instead.~~ | ~~No~~ |
| **clickPos** | ~~object~~ | ~~~~ | ~~DEPRECATED: data.clickPos is deprecated for clientEvent, use data.originPosition instead.~~ | ~~No~~ |

