...
- The `type` attribute must be set to `"program"` for this wire object.
- The `data` attribute defines the `program` object-specific attributes listed below.
...
| **channels** | [Channels](attr-channels)[] | `[{'path': '/ch/${scene}', 'type': 'pubsub', 'mode': 'rw', 'params': {'topic': 'realm/s/${scene}/${namespace}'}}]` | Channels describe files representing access to IO from pubsub and client sockets (possibly more in the future; currently only supported for WASM programs). | No |
| **run_info** | [RunInfo](attr-run_info) |  | Program execution info, added at runtime. | No |
